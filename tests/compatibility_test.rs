...
fn test_wfc_manager_data_structures() {
    ...
    for &cell in &cells {
        wfc_cell_data.insert(cell, CellWfcData {
...
/// 测试findIn2DVector兼容性
#[test]
fn test_find_in_2d_vector_compatibility() {
    // 模拟C++中的二维向量使用
    let layers = vec![
        vec![1, 2, 3],
        vec![4, 5, 6],
    ...
    // C++模式：auto [cx, cy] = findIn2DVector(layers, cell);
    let result = find_in_2d_vector(&layers, &5);
    assert_eq!(result, Some((1, 1)));
    
    let result = find_in_2d_vector(&layers, &10);
